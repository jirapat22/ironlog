...
function computeImprovedFlagsBatch(profileId, workout, exerciseIds) {
  ...
  // profile_id filtered here too (redundant given workout.id is always
  // pre-authorized by every caller before reaching this function) — kept
  // for the same reason priorRows above does: a query touching sets should
  // never rely solely on a caller's upstream check to stay profile-scoped.
  const thisSets = db.prepare(
    `SELECT id, exercise_id, weight, weight_unit, reps FROM sets
      WHERE workout_id = ? AND profile_id = ? AND is_warmup = 0 AND exercise_id IN (${ids.map(() => '?').join(',')})
      ORDER BY set_number ASC, id ASC`
  ).all(workout.id, profileId, ...ids);
